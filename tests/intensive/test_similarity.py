...
def test_pgvector_index_size():
    """
    Test that the index size in pgvector matches the expected number of embeddings.
    """
    dataset = foz.load_zoo_dataset(
        "quickstart",
        dataset_name="quickstart-test-pgvector-index-size",
        drop_existing_dataset=True,
    )
    ...
    # Remove some embeddings and verify the updated index size
    ids_to_remove = random.sample(list(index.sample_ids), 50)
    index.remove_from_index(sample_ids=ids_to_remove)
    assert index.total_index_size == len(dataset) - 50

    # Cleanup
    index.cleanup()
    dataset.delete_brain_run("pgvector_index_size")
    dataset.delete()


def test_pgvector_patch_similarity():
    """
    Test patch-level similarity search with pgvector.
    """
    dataset = foz.load_zoo_dataset(
        "quickstart",
        dataset_name="quickstart-test-pgvector-patch",
        drop_existing_dataset=True,
    )

    # Compute patch embeddings and index them using pgvector
    index = fob.compute_similarity(
        dataset,
        patches_field="ground_truth",
        model="clip-vit-base32-torch",
    ...
    # Verify index size
    assert index.total_index_size == dataset.count("ground_truth.detections")

    # Perform a similarity search
    prompt = "cute puppies"
    view = dataset.to_patches("ground_truth").sort_by_similarity(
        prompt, k=10, brain_key="pgvector_patch"
    )
    assert len(view) == 10

    # Cleanup
    index.cleanup()
    dataset.delete_brain_run("pgvector_patch")
    dataset.delete()
...
def test_pgvector_connection_handling():
    ...
    with pytest.raises(Exception):  # Expect an exception due to invalid connection
        index = fob.compute_similarity(
            dataset=None,
            model=None,
            metric="cosine",
