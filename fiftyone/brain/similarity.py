...
class DuplicatesMixin(object):
    ...
    def thresh(self):
        ...
        :meth:`find_unique` or :meth:`find_leaks`.
        """
        return self._thresh
    ...
    def unique_ids(self):
        ...
        or :meth:`find_unique` or :meth:`find_leaks`.
        """
    ...
    def duplicate_ids(self):
        ...
        :meth:`find_duplicates` or :meth:`find_unique` or :meth:`find_leaks`.
        """
    ...
    def neighbors_map(self):
        ...
        the last call to :meth:`find_duplicates` or :meth:`find_leaks`.
        """
    ...
    def split_views(self):
        """A dict mapping split names to views for the last call to
        :meth:`find_leaks`.
        """
    ...
    def find_leaks(self, splits, thresh):
        ...
        Args:
            ...
            thresh: the distance threshold to use to detect leaks
        """
        split_views, id2split = self._initialize_leaks(splits)

        self.find_duplicates(thresh=thresh)

        # Filter duplicates to just those with neighbors in different splits
        leak_ids = []
        for sample_id, neighbors in self._neighbors_map.items():
            _leak_ids = []

            sample_split = id2split.get(sample_id, None)
            if sample_split is None:
                continue

            for n in neighbors:
                neighbor_id = n[0]
                neighbor_split = id2split.get(neighbor_id, None)
    ...
    def leaks_for_sample(self, sample_or_id):
        ...
        leak_ids = []
        if sample_id in self._neighbors_map.keys():
            neighbors = self._neighbors_map[sample_id]
            leak_ids = [
                n[0] for n in neighbors if self._id2split[n[0]] != sample_split
            ]
        else:
            for unique_id, neighbors in self._neighbors_map.items():
        ...
        return self._samples.select([sample_id] + leak_ids, ordered=True)

    def no_leaks_view(self, view=None):
        """Returns a view with leaks excluded from the last call to
        :meth:`find_leaks`.
    ...
    def _initialize_leaks(self, splits):
        split_views = _to_split_views(self._samples, splits)
        id2split = {}

        split_ids = {}
        for split_name, split_view in split_views.items():
