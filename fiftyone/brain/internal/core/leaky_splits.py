...
class LeakySplitsSKLIndex(skl_sim.SklearnSimilarityIndex):
    ...
    def sort_by_leak_potential(self, sample, k=None):
        ...
        tags_to_search = copy(self.config.split_tags).remove(sample_split)
        self.use_view(self._curr_view.match_tags(tags_to_search))
        return self.sort_by_similarity(sample['id'], k)
